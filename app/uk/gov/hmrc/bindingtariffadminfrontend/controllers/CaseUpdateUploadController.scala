...
@Singleton
class CaseUpdateUploadController @Inject() (
  ...
  def get: Action[AnyContent] = authenticatedAction.async { implicit request =>
    successful(Ok(views.html.case_update_upload(form)))
  }

  def post(caseUpdateTarget: CaseUpdateTarget): Action[MultipartFormData[TemporaryFile]] =
    authenticatedAction.async(parse.multipartFormData) { implicit request =>
      ...
      request.body.file("file").filter(_.filename.nonEmpty) match {
        ...
        case Some(part: FilePart[TemporaryFile]) =>
          service
            .prepareCaseUpdates(toJsonSource(part.ref.path.toFile, part.contentType), priority, caseUpdateTarget)
            ...
            .recoverWith {
              ...
              case framing: FramingException =>
                ...
                successful(Ok(views.html.case_update_file_error(jsonErrors)))
              ...
              case JsResultException(errs) =>
                successful(Ok(views.html.case_update_file_error(errs)))
            }
      }
    }
