...
object CaseUpdate {
  object Mongo {
    private implicit def formatSetValue[A: Format]: OFormat[SetValue[A]] = Json.format[SetValue[A]]
    private implicit val formatNoChange: OFormat[NoChange.type]          = Json.format[NoChange.type]

    private implicit def formatUpdate[A: Format]: Format[Update[A]] =
      Union
        .from[Update[A]]("type")
    ...
    private implicit def formatBtiUpdate: OFormat[BTIUpdate] = {
    ...
    private implicit val formatLiabilityUpdate: OFormat[LiabilityUpdate] = Json.format[LiabilityUpdate]

    private implicit val formatApplicationUpdate: Format[ApplicationUpdate] = Union
