...
class CaseMigrationUploadControllerSpec extends ControllerSpec with BeforeAndAfterEach {
  ...
  "POST /" should {
    ...
    "Prepare Upload and Redirect To Migration State Controller with a plain JSON file" in {
      given(migrationService.prepareMigration(any[Source[MigratableCase, _]], refEq(false))(any[HeaderCarrier])) willReturn Future
        .successful(true)
    ...
    "Prepare Upload with priority flag" in {
      given(migrationService.prepareMigration(any[Source[MigratableCase, _]], refEq(true))(any[HeaderCarrier])) willReturn Future
        .successful(true)
    ...
    "Prepare Upload with minimal data with multiple appeals" in {
      ...
      val file =
        SingletonTemporaryFileCreator.create(withJson(fromFile("migration-minimal-all-appeal-fields-multiple.json")))
      val filePart = FilePart[TemporaryFile](key = "file", "file.txt", contentType = Some("text/plain"), ref = file)
      val form = MultipartFormData[TemporaryFile](
        dataParts = Map("priority" -> Seq("true")),
        files     = Seq(filePart),
    ...
    "return 200 with Json Errors" in {
      ...
      given(migrationService.prepareMigration(any[Source[MigratableCase, _]], any[Boolean])(any[HeaderCarrier])) willReturn Future
        .failed(jsonError)
