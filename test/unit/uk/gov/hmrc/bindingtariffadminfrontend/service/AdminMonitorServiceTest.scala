...
class AdminMonitorServiceTest extends UnitSpec with MockitoSugar with BeforeAndAfterEach {
  ...
  "Count Published Files" should {
    "Delegate to connector" in {
      given(fileConnector.find(refEq(Search(published = Some(true))))(any[HeaderCarrier])) willReturn Future.successful(Seq.empty)
  ...
  "Count Unpublished Files" should {
    "Delegate to connector" in {
      given(fileConnector.find(refEq(Search(published = Some(false))))(any[HeaderCarrier])) willReturn Future.successful(Seq.empty)
